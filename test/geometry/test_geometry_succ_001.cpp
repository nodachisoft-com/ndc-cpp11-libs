...
TEST(GeoCollision, isInsideOfDistance_Point_To_LineSegment_succ001)
{
  ...
  for (int v = 0; v < 100; v++)
  {
    for (int u = 0; u < 100; u++)
    {
      isInside = GeoCollision::isInsideOfDistance_Point_To_LineSegment(u, v, 10, 30, 75, 66, 7.0f);
      if (isInside)
      {
        // 茶色（道のイメージ）
...
TEST(GeoCollision, isInsideOfDistance_Point_To_Circle_succ001)
{
  ...
  for (int v = 0; v < 100; v++)
  {
    for (int u = 0; u < 100; u++)
    {
      isInside = GeoCollision::isInsideOfDistance_Point_To_Circle(u, v, 30, 40, 27.0f);
      if (isInside)
      {
        // 茶色（道のイメージ）
        ColorRGB color{219, 182, 55};
